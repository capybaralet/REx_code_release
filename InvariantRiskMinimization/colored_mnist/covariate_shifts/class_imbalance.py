...
parser.add_argument('--shift', type=str, default="shift_class_imbalance")
...
for restart in range(flags.n_restarts):
  ...
  def make_environment(images, labels, e, p, grayscale_dup=False):
    ...
    indices = torch.cat([label0_indices[:int(p*len(label0_indices))], label1_indices[:int((1-p)*len(label1_indices))]])

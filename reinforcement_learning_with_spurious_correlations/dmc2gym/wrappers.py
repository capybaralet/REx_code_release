...
class DMCWrapper(core.Env):
    ...
    def _get_obs(self, time_step):
        if self._from_pixels:
            ...
            obs_noise = np.zeros_like(obs)
            obs_noise[-noise_dims:] = np.random.randn(noise_dims) * self._noise
            spur_corr = np.matmul(obs[-noise_dims:], self._mult_factor)
            # spur_corr = obs[-1] * self._mult_factor
            obs = np.concatenate([obs + obs_noise, spur_corr])
        return obs
